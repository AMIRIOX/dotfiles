...
if [ -n "$DISPLAY_PLAYER" ]; then
    ...
    if [[ "$DISPLAY_PLAYER" == *"spotify"* ]]; then
        ...
        PLAYER_ICON="" # TODO: YesPlayMusic Icon
    fi
    ...
    if [ -n "$TITLE" ]; then
        if [ -n "$ALBUM" ]; then
            OUTPUT_TEXT="$PLAYER_ICON $STATUS_ICON $TITLE - $ARTIST"
        else
            OUTPUT_TEXT="$PLAYER_ICON $STATUS_ICON $TITLE"
        fi
    else
        OUTPUT_TEXT="$PLAYER_ICON $STATUS_ICON"
    fi
